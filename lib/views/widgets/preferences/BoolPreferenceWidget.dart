...
class BoolPreferenceWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PreferenceListTileWidget<BoolPreference>(
      initData: getIt.get<SharedPreferencesService>().loadBoolPreference(_defaultPreference), 
      listTileBuilder: (context, pref) => SwitchListTile(
        title: Text(pref.title),
        value: pref.value,
        onChanged: (value) => _notifyChange(BoolPreference(pref.key, pref.title, value)),
      )
    );
  }
}